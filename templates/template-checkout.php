...
<div id="checkout" data-product="<?php if (isset($_GET['id'])) {
                                        echo $_GET['id'];
    ...
    <form method="post" id="<?php echo $form_id; ?>" class="my-5 position-relative" action="<?php echo home_url() . $_SERVER['REQUEST_URI']; ?>">
        ...
        <div class="row">
            <div class="col-lg-5 order-lg-last">
                <div id="order_review" class="mb-4">
                    ...
                    <table class="table table-bordered bg-light" id="order_review_table">
                        <tbody>
                            <?php $i = 0;
                            foreach ($order_products_with_prices['products'] as $item) { ?>
                                <tr>
                                    <td>
                                        <?php echo $item['title']; ?>
                                        <?php if ($i === 0) { ?>
            ...
            <div class="col-lg-7">
                ...
                <p class="mt-4">
                    <input type="checkbox" name="is_company" value="1" id="is_company" <?php if (is_user_logged_in() && get_the_author_meta('is_company', $user_id)) {
                                                                                            echo 'checked';
                                                                                        } ?> />
                    <label for="is_company"><?php _e("Persoană juridică", 'goicc'); ?></label>
                </p>
                ...
                <div class="row">
                    <div class="col-lg-6">
                        <p class="form-username">
                            ...
                            <input class="text-input form-control" name="first_name" type="text" id="first_name" value="<?php if (is_user_logged_in()) {
                    ...
                    <div class="col-lg-6">
                        <p class="form-username">
                            ...
                            <input class="text-input form-control" name="last_name" type="text" id="last_name" value="<?php if (is_user_logged_in()) {
                ...
                ?>
                        <p class="form-field">
                            <label for="<?php echo $field; ?>"><?php echo $val['title']; ?> <?php if ($val['required']) {
                                                                                                echo '*';
                                                                                            } ?></label>
                            <input class="text-input form-control" name="<?php echo $field; ?>" type="text" id="<?php echo $field; ?>" value="<?php if (is_user_logged_in()) {
                ...
                ?>
                    <div id="payment-method" class="border boder-gray-300 bg-light p-3 mb-3">
                        ...
                        <?php if ($payment_method_count === 1) {
                            $payment_method = array_shift($payment_method); ?>
                        ...
                        <?php } else { ?>
                            <?php $i = 0;
                            foreach ($payment_method as $method => $val) { ?>
                                <hr>
                                <input type="radio" id="<?php echo $method; ?>" name="payment_method" value="<?php echo $method; ?>" <?php echo ($i === 0) ? "checked='checked'" : ""; ?>>
        ...
        <p class="form-submit">
            <?php //echo $referer; 
            ?>
            <input name="submit" type="submit" id="submit" class="submit button btn btn-primary" value="<?php _e('Submit', 'goicc'); ?>" />
            ...
            <input name="product_id" type="hidden" id="product_id" value="<?php if (isset($_GET['id'])) {
                                                                                echo $_GET['id'];
                                                                            } ?>" />
