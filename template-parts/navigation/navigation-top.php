...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
    <div class="container-xl">
        <a class="navbar-brand" href="<?php echo esc_url(get_bloginfo('url')); ?>">
            ...
            if (has_custom_logo()) { ?>
                <img src="<?php echo esc_url($logo[0]); ?>" class="d-inline-block custom-logo" alt="<?php echo esc_attr(get_bloginfo('name')); ?>">
            <?php } else { ?>
            ...
            <ul class="nav navbar-nav ">
                <?php if (is_user_logged_in()) { ?>
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle navbar-img d-flex align-items-center" data-toggle="dropdown" href="#" role="button" aria-haspopup="true" aria-expanded="false">
                            <?php _e('Hello', 'goicc'); ?>,
                            <?php
                        ...
                        <div class="dropdown-menu dropdown-dark dropdown-menu-right rounded-0 mb-3">
                            ...
                            if ($user_account_tabs && is_array($user_account_tabs)) {
                                ...
                                foreach ($user_account_tabs as $key => $tab) { ?>
                                    <a href="<?php echo $account_page . '?tab=' . $key; ?>" class="dropdown-item <?php if ($current_tab === $key) {
                                                                                                                        echo 'active';
                                                                                                                    } ?>">
                                        <?php echo $tab; ?>
                                    </a>
